...
function Siparis() {
  const[allSiparis, setAllSiparis] = useState([]);

  useEffect(() => { //sayfa açılır açılmaz çalışır bu kodlar
  
    const getAllSiparisInfo = async () => {
        let response = await axios.get(
            'https://private-cc8718-bahishali.apiary-mock.com/order'
            );
    
            //console.log("getAllUserInfo" + response.data.MusteriListesi);

            setAllSiparis(response.data.SiparisList);

    }

    // call the function
    getAllSiparisInfo().catch(console.error);
              ...
              <div className="portlet-body">
                <div className="table-responsive">
                  <table className="table table-striped table-bordered table-hover">
                    ...
                    <tbody>
                    {
                      allSiparis.map( (data) => (  //foreach gibi, data yerin index'te yazabilirsin
                      <> 
                        <tr>
